...
var widgetGeometry = {
  ...
  vertexTextureCoords: [],
  indices: [],
  indicesStrip: [], // indices for continouous triangle strip
  colors: []
};
...
function initSphereGeometry(size, rows) {
  size = size || 7;
  rows = rows || 16;
  ...
  for (var i = 0; i < rows; i++) {
    for (var j = 0; j < rows; j++) {
      widgetGeometry.indices.push(i + j * rows);
      widgetGeometry.indices.push(i + j * rows + 1);
      widgetGeometry.indices.push(i + j * rows + 2);
    }
  }
  ...
  widgetGeometry.indicesStrip = idxTriStrip;
